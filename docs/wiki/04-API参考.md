...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/admin/upload` | `multipart/form-data`，字段名 `file`；uuid 重命名落盘 `uploads/`；>5MB 返回 400；响应 `{url: "/uploads/xxx.png"}` |
...
- 上传：`yeyi_blog_upload_image(filename, content_base64)`；Base64 在解码前按 `MAX_UPLOAD_SIZE` 限制
