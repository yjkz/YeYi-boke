...
| 限流 | `middleware/rate_limit.py` 已实现 Redis 固定窗口限流器，**但当前尚未挂载到任何路由**（设计文档规划了评论 5 次/分等策略） |
| token 刷新 | 后端提供 `/auth/refresh`；admin 已存 refresh_token，**但 axios 拦截器当前 401 直接登出，未做静默刷新** |
