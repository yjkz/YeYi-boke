...
项目采用**前后端分离 + 模块化单体后端**架构。后端按业务域组织为 7 个模块（users / posts / comments / search / stats / config / mcp，每个模块拥有独立的 router / service / model / schema），为将来拆分微服务预留了边界。
...
| 数据库 | MySQL 8.0 | — | 主存储，12 张表（9 张业务表 + 3 张 MCP 管理表） |
...
- 承担：认证、文章/分类/标签 CRUD、评论、搜索、访问统计、站点配置、图片上传、RSS、MCP 管理 API（`/admin/mcp/*`）
...
graph LR
    subgraph app/modules
        ...
        mcp[mcp<br/>MCP 管理域<br/>设置/Key/审计日志]
    end
    ...
    main[main.py] --> users & posts & comments & config & search & stats & mcp
    main --> mw
    ...
    mcp --> redis & deps
    posts & comments & search & stats & config & users & mcp --> db
```
...
> 已知限制：防刷键中的 IP 取 `request.client.host`（未解析 `X-Real-IP`），网关反代后所有访客共享网关 IP，去重实际退化为**全站每文每小时最多 +1**。修复需改用 `X-Real-IP`，另行处理。

### 5.2 管理员发布文章

admin 登录获得 JWT → `POST /api/v1/admin/posts`（Bearer）→ `require_admin` 校验 → service 层：标题转拼音生成 slug、markdown-it 渲染 `content_html`、从渲染结果提取最多 200 字纯文本摘要 → 事务由 `get_db` 统一 commit → `POST /admin/posts/{id}/publish` 置为 published 并写 `published_at`。列表读取时会兼容并规范化历史 Markdown 源格式摘要。
...
`mcp` 是与 backend 分开运行的 MCP Streamable HTTP 服务，监听容器内 `8100` 端口，共享 MySQL、Redis 和 `uploads_data` 卷。公网入口为 `https://blogmcp.yeyeyiyi.online/mcp`，通过 `?tavilyApiKey=<key>`、`?api_key=<key>` 或 `X-MCP-API-Key` 请求头独立鉴权，不复用后台 JWT。
