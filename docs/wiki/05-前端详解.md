...
| 路由 | 文件 | 数据来源 | 说明 |
|------|------|----------|------|
| `/` | `pages/index.vue` | `GET /posts`、`GET /site/announcement` | 公告条 AnnouncementBar + PostCard 双列网格 + Pagination；分页用 `?page=` query |
...
| `/archive` | `pages/archive.vue` | `GET /posts?page_size=100` | 前端按发布年份分组的时间线列表 |
...
| `CommentSection` / `CommentItem` / `CommentForm` | 评论区容器（加载已批准评论 + 提交后 refresh）/ 嵌套回复展示 / 提交表单（昵称、邮箱、网站、内容、parent_id） |
| `SiteLeftSidebar` / `SiteRightSidebar` | Firefly 风格 widget 侧栏：站点信息、筛选、最新文章、统计、文章目录 |
| `TableOfContents` | H2/H3 文章目录内容 |
...
- `api/index.ts`：请求拦截器注入 Bearer；响应拦截器 401 → `auth.logout()` + 跳转 `/login`。
- > 现状说明：refresh_token 已保存且有 `/auth/refresh` 接口，但**未实现静默刷新**——access token 过期（2h）后直接登出重新登录。
...
| comments.ts | `commentsApi` | 评论列表/审核/删除 |
...
- 两个自定义上传：正文图上传后把 `![name](url)` 追加进 Markdown；封面上传直接回填 URL 字段
