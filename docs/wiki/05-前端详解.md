...
- 全局 head：中文标题、viewport、OG 标签、theme-color `#F8F7F4`；方圆体为构建时 `cn-font-split` 子集化的 woff2 分片（`npm run font:build` → `public/fonts/fangyuan/` + `assets/css/font-split.css`），经 unicode-range 按需加载，不再整包预加载
