...
- 全局 head：中文标题、viewport、OG 标签、theme-color `#F8F7F4`；方圆体为构建时 `cn-font-split` 子集化的 40 个 woff2 分片（共约 1.07MB，首屏按 unicode-range 只取数个小分片）：`npm run font:build` 产出 `public/fonts/fangyuan/` 与 `assets/css/font-split.css`（本地开发/新 clone 需先跑一次，分片产物不入 git），源 OTF 位于 `assets/fonts/` 不进公开产物，`nuxt.config` 已移除 2.66MB 整包 preload，网关对 `/fonts/` 返回一年 immutable 长缓存
...
│   ├── useVisit.ts            # 访问埋点
│   ├── useAnnouncementModal.ts # 公告完整版弹窗的开关与触发元素记录
│   ├── useRecentPostsModal.ts  # “查看全部文章”弹窗的开关与触发元素记录
│   └── useContentEnhancements.ts # 正文增强渲染：DOMPurify 清洗 + KaTeX/Prism
├── components/                # 19 个自动导入组件
├── pages/                     # 文件路由
│   ├── index.vue              # 首页
│   ├── posts/[slug].vue       # 文章详情
...
- 侧栏数据使用 `GET /categories`、`GET /tags`、`GET /site/announcement`、`GET /posts?page=1&page_size=5&sort=latest` 和公开 `GET /stats/summary`，通过 `useAsyncData` 缓存；“查看全部”打开 `RecentPostsModal`，按 `sort=latest` 分页展示全部已发布文章。
...
首页和搜索页使用 `post-grid` 自动填充网格：单篇结果限制为不超过 `48rem` 并靠左对齐，多篇结果按最小稳定列宽分栏，避免单条文章被拉伸或宽桌面出现无意义的大块空洞。
...
| 组件 | 职责 |
|------|------|
| `AppHeader` / `AppFooter` | 页头导航（含 SearchBox、DarkToggle）/ 页脚（footer_text + 公安备案链接与图标 + social_links） |
...
| `SiteLeftSidebar` / `SiteRightSidebar` | Firefly 风格 widget 侧栏：站点信息、筛选、最新文章、统计、文章目录；最新文章支持“查看全部”分页弹窗 |
| `RecentPostsModal` / `useRecentPostsModal` | 全部文章弹窗：按发布日期从新到旧分页，支持背景点击、Esc 和焦点返回 |
...
`npm run build` 产出 `.output/`；Docker 运行阶段直接 `node .output/server/index.mjs` 启动 Node SSR 服务（端口 3000）。无需 Nginx 托管静态文件。Docker builder 阶段使用 `node:24-slim` 并在 `npm run build` 前执行 `npm run font:build`（cn-font-split 原生内核不兼容 alpine/musl，内核 7.6.8 显式下载并做 sha256 校验），详见 [06-部署与运维](./06-部署与运维.md)。

已知取舍：Nuxt `inlineStyles` 会把全局 CSS（含 `font-split.css`）内联进 SSR HTML，每页约增 8-10KB（gzip）；改为外链属后续优化。
...
├── api/                  # axios 封装 + 8 个领域 API 模块
│   ├── index.ts auth.ts posts.ts categories.ts tags.ts comments.ts config.ts stats.ts mcp.ts
├── layouts/AdminLayout.vue   # 侧边栏骨架（9 个菜单项）
└── views/                # 11 个页面组件
```
...
| mcp.ts | `mcpApi` | MCP 概览/设置/Key 管理/日志查询与清理 |
...
**AdminLayout.vue**：Element Plus 容器；移动端抽屉式侧边栏；挂载时拉取站点配置动态设置 favicon 与 `document.title`；顶栏提供“前往 Blog”入口，地址由构建参数 `VITE_BLOG_URL` 注入。
