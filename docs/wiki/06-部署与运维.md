...
| `blogmcp.yeyeyiyi.online` | `/mcp`、`/health` → mcp:8100 | — | — |

- blog/admin 两个 server 均附加安全头：`X-Frame-Options: SAMEORIGIN`、`X-Content-Type-Options: nosniff`、`Referrer-Policy`
- `client_max_body_size 10m`（大于后端 5MB 上传限制，富余）
...
| frontend | `frontend/Dockerfile` | node:24-alpine `npm ci && npm run build` | `node .output/server/index.mjs`（Node SSR） |
| admin | `admin/Dockerfile` | node:24-alpine `npm run build`（vue-tsc 类型检查） | nginx:alpine 托管 dist + 反代配置 |
...
- 所需 secrets：`ACR_PASSWORD`、`SERVER_IP`、`SSH_KEY`、`JWT_SECRET_KEY`、`MYSQL_ROOT_PASSWORD`
